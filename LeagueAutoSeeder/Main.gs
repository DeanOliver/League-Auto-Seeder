function LoLSeeder() {
  ...
  for(n=1;n<sheet.length;++n){                          // Create a player object for all the RIOT IDs.
    var teamName = sheet[n][0];                         // n=row, x=column
    var riotID = sheet[n][1];                           // n=row, x=column   
    var player = CreatePlayerObject(riotID); // Create a player with all their ranked info.
  ...
  for(i=0;i<(teams.length);++i){
    for(p=0; p<teams[i].players.length;++p){
      ...
      activeSheet.getRange(count,2).setValue([teams[i].players[p].riotID]);
      activeSheet.getRange(count,3).setValue([teams[i].players[p].gameName]);
      activeSheet.getRange(count,4).setValue([teams[i].players[p].tagLine]);
      activeSheet.getRange(count,5).setValue([
      teams[i].players[p].tier + " " + 
      teams[i].players[p].division + " " + 
      teams[i].players[p].lp + "LP"]);
      activeSheet.getRange(count,6).setValue([teams[i].players[p].rankValue]);  
      activeSheet.getRange(count,7).setValue([teams[i].avrageRankScore]);
      count++;
    }
  }
  return;
